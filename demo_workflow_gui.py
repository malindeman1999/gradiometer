...
def step1_build_grid_admittance(subdiv: int, lmax: int, checker_divisions: int, log) -> Path:
    ...
    cond = _checkerboard_admittance(grid.positions.to(torch.float64), 0.0, sigma_2d_max, checker_divisions)
...
def step4_render_overview(log) -> Path:
    payload = _load_state("solution_latest.pt")
    sim_out: PhasorSimulation = payload["phasor_sim"]
    label: str = payload.get("label", "self_consistent")
...
def step4_render_gradient(altitude_m: float, log) -> Path:
    payload = _load_state("solution_latest.pt")
    sim_out: PhasorSimulation = payload["phasor_sim"]
    label: str = payload.get("label", "self_consistent")
    grid_state = _load_state("grid_admittance.pt")
...
def main():
    ...
    log_widget.grid(row=7, column=0, columnspan=10, pady=8, sticky="nsew")
    ...
    btn_step1b.grid(row=1, column=13, padx=6)

    # Step 1c
    ttk.Label(frm, text="Step 1c: Roundtrip stability").grid(row=2, column=0, sticky="w")
    ...
    btn_step1c.grid(row=2, column=13, padx=6)

    # Step 2
    ttk.Label(frm, text="Step 2: Ambient field").grid(row=3, column=0, sticky="w")
    btn_step2 = tk.Button(
        frm,
        text="Build ambient",
        command=lambda: run_step(btn_step2, lambda: step2_build_ambient(lambda msg: _log(log_widget, msg))),
    )
    btn_step2.grid(row=3, column=13, padx=6)

    # Step 3
    ttk.Label(frm, text="Step 3: Solve currents").grid(row=4, column=0, sticky="w")
    btn_selfc = tk.Button(
        frm,
        text="Self-consistent",
        command=lambda: run_step(btn_selfc, lambda: step3_solve_currents(False, lambda msg: _log(log_widget, msg))),
    )
    btn_selfc.grid(row=4, column=13, padx=4, sticky="w")
    btn_first = tk.Button(
        frm,
        text="First-order only",
        command=lambda: run_step(btn_first, lambda: step3_solve_currents(True, lambda msg: _log(log_widget, msg))),
    )
    btn_first.grid(row=4, column=14, padx=4, sticky="w")

    # Step 4
    ttk.Label(frm, text="Step 4: Plots / gradients").grid(row=5, column=0, sticky="w")
    btn_overview = tk.Button(
        frm,
        text="Overview plots",
        command=lambda: run_step_ui(btn_overview, lambda: step4_render_overview(lambda msg: _log(log_widget, msg))),
    )
    btn_overview.grid(row=5, column=12, padx=4, sticky="w")
    btn_grad0 = tk.Button(
        frm,
        text="Gradients @ surface",
        command=lambda: run_step_ui(btn_grad0, lambda: step4_render_gradient(0.0, lambda msg: _log(log_widget, msg))),
    )
    btn_grad0.grid(row=5, column=13, padx=4, sticky="w")
    btn_grad100 = tk.Button(
        frm,
        text="Gradients @ 100 km",
        command=lambda: run_step_ui(btn_grad100, lambda: step4_render_gradient(100e3, lambda msg: _log(log_widget, msg))),
    )
    btn_grad100.grid(row=5, column=14, padx=4, sticky="w")

    frm.rowconfigure(7, weight=1)
    frm.columnconfigure(15, weight=1)
